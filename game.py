...
def draw():
    global items, game_complete, current_level, game_over
    screen.clear()
    screen.blit("bg", (0,0))
    if game_over:
        display_message("GAME OVER", "Try again")
    elif game_complete:
        display_message("YOU WON","Great job, recycler")
    else:
        for item in items:
            item.draw()
...
def display_message(main_text,sub_text):
    screen.draw.text(main_text,fontsize= 60 , center = CENTRE, color = "blue")
    screen.draw.text(sub_text,fontsize = 40 ,center = (CENTRE_X,CENTRE_Y + 30),color = "purple" )
